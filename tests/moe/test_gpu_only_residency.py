...
def test_dsv4_plain_weights_follow_engine_compute_dtype(monkeypatch):
    import freetoken.distributed.info as info
    from freetoken.distributed import DistributedInfo
    from freetoken.models.deepseek_v4.args import DeepseekV4Args
    from freetoken.models.deepseek_v4.model import Transformer
    ...
    assert model.head.dtype == torch.float16
    assert layer.attn.wo_a.dtype == torch.float16
    assert layer.ffn.gate.weight.dtype == torch.float16


@pytest.mark.skipif(not torch.cuda.is_available(), reason="needs CUDA")
def test_dsv4_tp4_partial_expert_outputs_sum_to_unsharded_fp16():
    """Exercise the production SM75 dtype and TP=4 local-I geometry.
    ...
    x = torch.randn(3, H, dtype=torch.float16, device=device) * 0.1
