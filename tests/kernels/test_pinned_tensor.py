...
def test_host_device_ptr_is_identity_under_uva():
    ...
    # CUDA 13 validates registration even on Linux/UVA; older runtimes sometimes
    # returned the host VA for arbitrary pageable pointers. Only exercise the
    # supported contract (pinned + mapped) so a rejected invalid-pointer probe
    # cannot leave a sticky CUDA error for later tests.
